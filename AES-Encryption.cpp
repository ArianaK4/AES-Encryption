...
//Inserting RCON (the exponentiation of 2 to a user-specified value) S-Box table:
...
void KeyExpansionCore(unsigned char* in, unsigned char i)
{
	...
	*/
	...
	//S-Box, four bytes:
	//in[n] = s_Box[in[n]] => in[n] value becomes whichever value is in[n] in s_Box:
	in[0] = s_Box[in[0]];
	in[1] = s_Box[in[1]];
	in[2] = s_Box[in[2]];
	in[3] = s_Box[in[3]];
...
void keyExpansion(unsigned char* inputKey, unsigned char* expandedKeys) 
{
	//Key Expansion: 
		...
		//the first 16 bytes of the expanded keys are the same as the origial key so we copy over:
	for (int i = 0; i < 16; i++)
		expandedKeys[i] = inputKey[i];

	//Variables we'll use:
	int bytesGenerated = 16; //we have generated 16 bytes so far in the two lines above ^^
	int rconIteration = 1; //RCon iteration begins at 1
	unsigned char temp[4]; //temporary storage for core 

	//Main loop of the key expansion (while we still got bytes to generate, read the bytes from core):
	while (bytesGenerated < 176)
	{
		//Read 4 bytes for the core and store them in temporary array:
		for (int i = 0; i < 4; i++)
			temp[i] = expandedKeys[i + bytesGenerated - 4];

		//Perform the core once for each 16 byte key (once every time we've got to generate a new key):
		...
		//XOR temp with [bytesGenerated-16] and store in expanded keys:
			//xor the first 4 bytes we generated from core with the temp value and store them as the next 4 generated bytes:
